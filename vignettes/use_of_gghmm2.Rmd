...
Hidden Markov model (HMM) is a statistical Markov model where the system in question is being model by a Markov process $C$ with hidden states. In other words $C$ - which will be called the hidden state -  can not be observed directly so we instead model on $X$ which from now on will be called the observable states. Thus we learn about $C$ at $t=t_0$ from $X$ at $t=t_0$.
