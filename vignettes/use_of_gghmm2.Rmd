...
\section{The package}
To see how the package works, we will cover a example of it, using the earthquakes data contained in the package. First we load that data set and build the model we will be using. We will be trying to fit a $2$ state hidden markom model where the states emit from a poisson distribution. So we set *emission_function_names* to "dpois" to tell the model that we 
...
HM = HMM(initial_dist = delta,transmission = trans,
         emission_function_names = "dpois",parm1 = c(10,30) )
HM
```
...
HMN = HMM(initial_dist = delta,transmission = trans,
         emission_function_names = "dnorm",parameterlist = list(list(mean=10,sd=1),list(mean=20,sd=1)) )
HMN
HMN2 = em(HMN,X)
HMN2
```
